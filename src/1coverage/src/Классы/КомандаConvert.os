...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("ibconnection", "", "строка соединения с информационной базой")
				.ТСтрока()
				.ВОкружении("1COVERAGE_IBCONNECTION")
				.Обязательный(Истина);
...
Процедура ЗаполнитьДанныеПокрытия(ЗаписиЛогаJSON)
	...
	Для Каждого ТекЗаписьЛогаJSON Из ЗаписиЛогаJSON Цикл
		...
		Если Не ЭтоВалиднаяЗапись(ТекЗаписьЛогаJSON) Тогда
			Сообщить("ОТЛАДКА: запись лога не валидна, отсутствуют необходимые поля");
			Продолжить;
		КонецЕсли;
		...
		Для Каждого Запись Из ДанныеМодуля Цикл
			...
			Сообщить("ОТЛАДКА: нашли в замерах модуль с id " + ИдМодуля);
...
Процедура ОпределитьОбъектыКонфигурации()
	...
	Если ФС.ФайлСуществует(ПутьКФайлуКонфигурации) Тогда
		...
		Сообщить("ОТЛАДКА: Конфигурация в формате XML");
		ИсходникиВФорматеXML = Истина;
		ОпределитьОбъектыКонфигурацииXML();
		
	ИначеЕсли ФС.ФайлСуществует(ПутьКФайлуКонфигурацииEDT) Тогда
		
		Сообщить("ОТЛАДКА: Конфигурация в формате EDT");
		ИсходникиВФорматеXML = Ложь;
		ОпределитьОбъектыКонфигурацииEDT();
		
	Иначе
		
		Сообщить("ОТЛАДКА: Не найден исходный код конфигурации или он выгружен в неизвестном формате");
		Возврат;
...
Процедура ОпределитьОбъектыКонфигурацииXML()
	...
	Для Каждого ТекФайл Из МассивФайлов Цикл
		...
		Если ТекUUID = Неопределено Тогда
			Сообщить("ОТЛАДКА: возможно, ошибка");
			Продолжить;
		КонецЕсли;
...
Процедура ОбработатьДочерниеОбъектыXML(УзелОбъектМетаданных, ОтносительныйПуть, ТекФайл)
	...
	Иначе
		Сообщить("ОТЛАДКА: поведение не опредлено");
	КонецЕсли;
...
Процедура ЗаписатьМодульВДанныеПокрытия(UUID, ОтносительныйПуть, ТекИмяФайла, ИмяКоманды = Неопределено)
	...
	Если ДанныеПокрытияМодуляКонфигурации = Неопределено Тогда
		Сообщить("ОТЛАДКА: здесь не было замеров " + UUID);
	Иначе
		
		Сообщить("ОТЛАДКА: здесь БЫЛ замер " + UUID);
...
Процедура ПостроитьПутиКМодулямКонфигурацииXML()
	...
	Для Каждого Стр Из ДанныеПокрытия Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.ПутьКОбъекту) Тогда
			Сообщить("ОТЛАДКА: не удалось определить объект с Id модуля " + Стр.ИдМодуля);
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли Стр.ИдТипаМодуля = ТипыМодулей["Неизвестно"] Тогда
			Сообщить("TODO");
		Иначе
			Сообщить("ОШИБКА: Неизвестный идентификатор типа модуля");
		КонецЕсли;
...
Процедура ПостроитьПутиКМодулямКонфигурацииEDT()
	...
	Для Каждого Стр Из ДанныеПокрытия Цикл
		...
		Если Не ЗначениеЗаполнено(Стр.ПутьКОбъекту) Тогда
			Сообщить("ОТЛАДКА: не удалось определить путь к объекту с Id " + Стр.ИдМодуля);
			Продолжить;
		КонецЕсли;
...
Процедура СохранитьВGenericCoverage(КаталогРезультатов)
	...
	Для Каждого Модуль Из ДанныеПокрытия Цикл
		...
		Если ПустаяСтрока(Модуль.ПутьКМодулю) Тогда
			Сообщить("Не удалось определить путь к модулю " + Модуль.ИдМодуля + " с типом " + Модуль.ИдТипаМодуля);
			Продолжить;
		КонецЕсли;
	...
	Сообщить("Количество строк в замерах = " + КоличествоСтрок);
...
Процедура ВывестиРезультат()
	...
	Для Каждого СтрТЗ Из ДанныеПокрытия Цикл
		
		// Сообщить(СтрТЗ.ИдМодуля + "|" + СтрТЗ.ИдТипаМодуля + "|" + СтрТЗ.ПутьКОбъекту + "|" + СтрТЗ.ПутьКМодулю);
		
		Сообщить("" + СтрТЗ.ПутьКОбъекту + "  |  " + СтрТЗ.ПутьКМодулю);
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ВывестиСоответствие(ТекСоответствие)
	
	Для Каждого Элемент Из ТекСоответствие Цикл
...
Процедура ПроверитьСуществованиеФайлов()
	...
	Для Каждого Стр Из ДанныеПокрытия Цикл
		...
		Если Не ФС.ФайлСуществует(Стр.ПутьКМодулю) Тогда
			Сообщить("Файл не существует:" + Стр.ПутьКМодулю);
		КонецЕсли;
